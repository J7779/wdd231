...
const API_KEY = "dd019121ecd3beb483cf8b1ff9cce3fd";
...
function createwassupCard(index) {
    const wassupCard = document.createElement("div");
    wassupCard.className = `card-stack card-stack-0${index}`;

    wassupCard.innerHTML = `
        <div class="title-wassup">
            <h4 id="business-name-0${index}"></h4>
            <h3 id="tag0${index}"></h3>
        </div>
        <div class="wassup-img">
            <img src="" alt="" id="img-0${index}-wassup" width="80px">
        </div>
        <div class="wassup-data">
...
async function populatewassupCards() {
    const wassuplightsMainBox = document.querySelector(".wassuplights-main-box");
    if (!wassuplightsMainBox) return;

    wassuplightsMainBox.innerHTML = "";
    for (let i = 1; i <= 3; i++) {
        wassuplightsMainBox.appendChild(createwassupCard(i));
    }
    ...
    const wassupCardData = shuffledData.slice(0, 3);

    wassupCardData.forEach((member, index) => {
        ...
        const imgElement = document.querySelector(`#img-0${cardIndex}-wassup`);
...
async function fetchAndDisplayCurrentWeather() {
    const urlWeather = `https://api.openweathermap.org/data/2.5/weather?lat=${LATITUDE}&lon=${LONGITUDE}&appid=${API_KEY}`;
...
async function fetchAndDisplayWeatherForecast() {
    ...
    dailyForecasts.forEach((dailyData, index) => {
        ...
        if (tempElement) {
            tempElement.textContent = `${parseFloat(dailyData.main.temp).toFixed(0)}°F`;
        }
    });
}
